...
This repository is a consumer of the agent-os v1.1.0 control plane, loaded
into Pi as a TypeScript extension.

Unlike the v3 Python era (when this doc first landed), governance files
**are vendored** at the agent-os v1.1.0 tag. v1.1.0's
...
listed in `.agent-os/contracts/index.json`. The `/init` command
fetches all of these as raw bytes from the v1.1.0 tag — they are not
...
- This project's `.agent-os/project.yaml` declares
  ...
  `memory_policy.global_memory_write: false`. v1.1.0 does not enforce these
  fields yet; they document the project's intent for future releases that
...
- All harness adapters (`AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`)
  ...
  the brain via MCP but cannot drive the CCP slash-command loop in v1.1.0.
